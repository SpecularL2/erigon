...
type TestProver struct {
	prover *specularprover.TestProver
	rules  *chain.Rules
	dbtx   kv.Tx
...
func NewTestProver(
	...
	prover := specularprover.NewTestProver(
		step,
		transaction,
	...
	return &TestProver{prover, rules, dbtx, reader, writer}, nil
}

func (p *TestProver) CaptureTxStart(gasLimit uint64) {
	p.prover.CaptureTxStart(gasLimit)
}

func (p *TestProver) CaptureTxEnd(restGas uint64) {
	p.prover.CaptureTxEnd(restGas)
}

func (p *TestProver) CaptureStart(env vm.VMInterface, from libcommon.Address, to libcommon.Address, precompile bool, create bool, input []byte, gas uint64, value *uint256.Int, code []byte) {
...
func (p *TestProver) CaptureEnd(output []byte, usedGas uint64, err error) {
	p.prover.CaptureEnd(output, usedGas, 0, err)
}

func (p *TestProver) CaptureEnter(typ vm.OpCode, from libcommon.Address, to libcommon.Address, precompile bool, create bool, input []byte, gas uint64, value *uint256.Int, code []byte) {
	p.prover.CaptureEnter(gethvm.OpCode(typ), gethcommon.Address(from), gethcommon.Address(to), input, gas, value.ToBig())
}

func (p *TestProver) CaptureExit(output []byte, usedGas uint64, err error) {
	p.prover.CaptureExit(output, usedGas, err)
}

func (p *TestProver) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	scopew := &ErigonScopeContextWrapper{scope}
	p.prover.CaptureStateImpl(pc, gethvm.OpCode(op), gas, cost, scopew, rData, depth, err)
}

func (p *TestProver) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, depth int, err error) {
	scopew := &ErigonScopeContextWrapper{scope}
	p.prover.CaptureFaultImpl(pc, gethvm.OpCode(op), gas, cost, scopew, depth, err)
}

func (p *TestProver) GetResult() ([]byte, error) {
